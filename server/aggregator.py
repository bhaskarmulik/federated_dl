...
#     if use_tls:
#         with open("./server_tls/server.crt","rb") as f: 
#             cert = f.read()
#         with open("./server_tls/server.key","rb") as f: 
#             key = f.read()
#         with open("./client_tls/client.crt","rb") as f: 
#             ca = f.read()
#         creds = grpc.ssl_server_credentials([(key, cert)], root_certificates=ca, require_client_auth=False)
...
def load_initial_global(path="models/global_model.pt"):
    ...
    if os.path.exists(path):
        ...
        print(f"[Server] Loaded global weights from {path}")
        return
    print(f"[Server] Loaded global weights")
    model = models.resnet18(num_classes=2)
    buf = io.BytesIO()
    torch.save(model.state_dict(), buf)
    GLOBAL_SD_BYTES = buf.getvalue()
...
def serve(bind_addr="0.0.0.0:50051", use_tls=False):
    ...
    threading.Thread(target=aggregate_periodically, daemon=True).start()
    server.start()
    print(f"[Server] listening on {bind_addr[-1:-5]} TLS={use_tls}")
    server.wait_for_termination()
